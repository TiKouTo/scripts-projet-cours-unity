    ...
    public class MoveToCopie : MonoBehaviour {
      ...
      public bool when_arrived_actualise_position_once;
      ...
      public void Init(List<ScriptableObject> targets) //fais par Chatgpt , fonction d'initialisation
      {
         ...
         when_arrived_actualise_position_once = false;
         ...
         if(!doing_the_attraction)
         {
            ...
            else
            {
               ...
               if(I_am_next) 
               {
                  ...
                  if(f_arrived(((attraction)list_targets[actual_target]).point_d_interet.position)) 
                  {
                     if(!when_arrived_actualise_position_once)
                     {
                        ((attraction)list_targets[actual_target]).list_d_attente[i_celui_avant_moi_dans_la_queue] = agent_007_au_rapport;
               ...
               {
                  if(f_arrived(((attraction)list_targets[actual_target]).list_d_attente[i_celui_avant_moi_dans_la_queue].my_position + offset_position))
                  {
                     ((attraction)list_targets[actual_target]).list_d_attente[i_celui_avant_moi_dans_la_queue] = agent_007_au_rapport;
                     when_arrived_actualise_position_once = true;
                  }
